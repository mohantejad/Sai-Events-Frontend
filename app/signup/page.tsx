...
const Signup = () => {
  ...
  const onSubmit: SubmitHandler<SignupFormInputs> = async (data) => {
    try {
      const response = await fetch("https://sai-events-backend-simplified.onrender.com/auth/signup/", {
        method: "POST",
      ...
      <div className="bg-white border border-[#81a7e3] rounded-lg p-8 w-full max-w-md">
        ...
        <form className="mt-6 space-y-4" onSubmit={handleSubmit(onSubmit)}>
          <div className="flex flex-col space-y-1">
            <label htmlFor="username" className="text-[#004aad]/90">
              Username
            </label>
            <input
              id="username"
              ...
              className={`w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 ${
                errors.username ? "border-red-500 focus:ring-red-500" : "border-[#004aad]/90 focus:ring-[#004aad]"
              }`}
